...
@section('content')
    <h2>Status Laundry Saya</h2>

    @php
        $transaksiTerbaru = $transaksi->sortByDesc('tanggal_masuk')->first();
    @endphp

    <div class="page-card" style="margin-bottom: 18px;">
        <div style="display:flex; flex-wrap:wrap; gap:12px;">
            <div style="min-width: 220px;">
                <div style="font-weight: 900; margin-bottom: 6px;">Status laundry terbaru</div>
                <div>
    ...
    <div id="report-modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:100; align-items:center; justify-content:center;">
        <div style="background:#fff; border-radius:18px; padding:22px; width:90%; max-width:480px; box-shadow:0 20px 50px rgba(0,0,0,.2);">
            ...
            <form id="report-form">
                @csrf
                <input type="hidden" name="page_name" value="Status Laundry">
    ...
    @if ($transaksi->isEmpty())
        <p>Belum ada transaksi laundry yang tercatat untuk akun ini.</p>
    @else
        <table class="table">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach($transaksi as $item)
                    <tr>
                        ...
                        <td>{{ $item->statusLaundry?->nama_status ?? $item->status ?? 'Dalam proses' }}</td>

        ...
        <table class="table">
            ...
            <tbody>
                @foreach($transaksi as $t)
                    <tr>
<td>{{ $t->kode_transaksi ?? $t->transaksi_id }}</td>
                        <td>{{ $t->layanan?->nama_layanan ?? '-' }}</td>
                        <td>{{ number_format($t->total_harga,0,',','.') }}</td>
                        <td>{{ $t->statusLaundry?->nama_status ?? $t->status_laundry ?? 'Dalam proses' }}</td>
                        <td>{{ $t->tanggal_transaksi }}</td>
                        <td><a href="{{ route('pelanggan.transactions.show', $t->id) }}">Lihat</a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div id="report-modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:100; align-items:center; justify-content:center;">
        <div style="background:#fff; border-radius:18px; padding:22px; width:90%; max-width:480px; box-shadow:0 20px 50px rgba(0,0,0,.2);">
            ...
            <form id="report-form">
                @csrf
                <input type="hidden" name="page_name" value="Riwayat Transaksi">
